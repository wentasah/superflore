...
class NixPackage:
    ...
    def __init__(
        ...
        rosinstall = _generate_rosinstall(
            name, repo.url, repo.get_release_tag(name), True
        )
        ...
        archive_path = os.path.join(
            tar_dir,
            '{}-{}-{}.tar.gz'.format(self.normalize_name(name), version, distro.name),
        )
        ...
        if package_xml is None:
            ...
            package_xml = retry_on_exception(ros_pkg.get_package_xml, distro.name)

        metadata = PackageMetadata(
            package_xml, NixPackage._get_condition_context(distro.name)
        )

        dep_walker = DependencyWalker(distro, get_distro_condition_context(distro.name))
    ...
    def _resolve_dependencies(self, deps: Iterable[str]) -> Set[str]:
        return set(itertools.chain.from_iterable(map(self._resolve_dependency, deps)))
    ...
    def _get_condition_context(distro):
        ...
        context['ROS_PYTHON_VERSION'] = str(NixPackage._get_ros_python_version(distro))
        return context
