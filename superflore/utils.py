...
def load_pr():
    try:
        ...
        err('Please supply the %s and %s files' % ('.pr-message.tmp', '.pr-title.tmp'))
        raise
    return msg, title
...
def file_pr(overlay, delta, missing_deps, comment, distro=None, title=''):
    try:
        ...
        err(
            ...
            % (overlay.repo.repo_owner, overlay.repo.repo_name)
        )
...
def get_distros_by_status(status='active'):
    return [
        t[0] for t in get_distros().items() if t[1].get('distribution_status') == status
    ]
...
def download_file(url, filename):
    ...
    request = urllib.request.Request(
        url, headers={'User-Agent': 'superflore/{}'.format(get_superflore_version())}
    )
    try:
    ...
    *args,
    ...
    try:
        ...
        if num_retry >= max_retries or max_retries < 0 or num_retry < 0:
            if error_msg:
                err('{0} {1} {2}/{3}'.format(str(e), error_msg, num_retry, max_retries))
            raise e from None
        if num_retry > 0:
            if retry_msg:
                warn(
                    '{0} {1} {2}/{3}...'.format(
                        str(e), retry_msg, num_retry, max_retries
                    )
                )
